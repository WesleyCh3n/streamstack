...
OPENCV_CONTRIB_TAR = opencv_contrib.tar.gz
OPENCV_CONTRIB_SRC_DIR = opencv_contrib-src

all:
	@[ -s $(OPENCV_TAR) ] || curl -L $(OPENCV_SRC_URL) -o $(OPENCV_TAR)
	@[ -s $(OPENCV_CONTRIB_TAR) ] ||  curl -L  $(OPENCV_CONTRIB_SRC_URL) -o $(OPENCV_CONTRIB_TAR)
	@mkdir -p $(OPENCV_SRC_DIR) && mkdir -p $(OPENCV_CONTRIB_SRC_DIR)
	@tar xf $(OPENCV_TAR) --strip-components=1 -C $(OPENCV_SRC_DIR)
	@tar xf $(OPENCV_CONTRIB_TAR) --strip-components=1 -C $(OPENCV_CONTRIB_SRC_DIR)
	@cmake \
		...
		-DBUILD_LIST=core,dnn,videoio,cudev,cudacodec \
		-B $(BUILD_DIR) \
		$(OPENCV_SRC_DIR)
	@echo "cmake --build $(BUILD_DIR) -- -j$$(($(shell nproc --all) / 2))"
